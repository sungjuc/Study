  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      Observable(observer => {
        ...
        // If input stream has an exception, it will not be completed. So we have to complete the output stream here.
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      obs takeUntil Observable.interval(totalSec second)
    }
